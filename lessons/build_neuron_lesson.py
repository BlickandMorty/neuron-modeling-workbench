"""Build and execute the compartmental NEURON lesson."""
...
NOTEBOOK = ROOT / "06_neuron_compartments.ipynb"
EXECUTED = ROOT / "06_neuron_compartments.executed.ipynb"

cells = [
    nb.v4.new_markdown_cell(
        "# NEURON: soma, dendrite, and spatial resolution\n\n"
        "Use the **Science - NEURON** kernel. This is a software-verification and "
        "cable-model lesson, not a fitted human neuron or clinical model. The built-in "
        "`hh` mechanism is historically squid-axon-derived."
    ),
    nb.v4.new_code_cell(
        "from pathlib import Path\n"
        "import json\n"
    ...
    nb.v4.new_markdown_cell(
        ...
        "AI checkpoint prediction: a brief soma current will trigger an active spike; "
        "the passive dendritic tip will depolarize with attenuation and delay. Increasing "
        "spatial resolution while holding the time step fixed should change the trace slightly, not reverse "
        "that qualitative result. Replace this with your own prediction before changing parameters."
    ),
    nb.v4.new_code_cell(
    ...
    nb.v4.new_code_cell(
        ...
        "np.savetxt(result_dir / 'baseline_voltage.csv', np.column_stack(baseline), "
        "delimiter=',', header='time_ms,soma_mv,middle_mv,tip_mv', comments='')\n"
    ...
    nb.v4.new_markdown_cell(
        ...
        "Explain why the soma and dendritic tip differ even though they are connected. "
        "Then identify which claims are numerical (the resolution comparison), mechanistic "
        "(active soma versus passive dendrite), and out of scope (human cell identity, learning, "
        "mental health, or drug response).\n\n"
        "Next experiment: write a prediction, copy the fixed baseline, then change exactly one "
        "of dendrite length, diameter, passive conductance, or stimulus amplitude."
    ),
]
...
executed = NotebookClient(
    notebook,
    kernel_name="science-neuron",
    timeout=180,
    resources={"metadata": {"path": str(ROOT)}},
).execute()
nb.validate(executed)
nb.write(executed, EXECUTED)
print(NOTEBOOK)
print(EXECUTED)
